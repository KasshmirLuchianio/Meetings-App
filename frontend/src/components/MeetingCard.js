...
import { MapPin, Calendar, ChevronRight, Trash2, Pencil, X, Check } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function MeetingCard({ meeting, onDeleted, onUpdated }) {
  ...
  // Regular tap/click — navigate only if menu is closed and no long press
  ...
  const handleSaveTitle = async () => {
    ...
    try {
      ...
      toast.error('Nu s-a putut salva: ' + err.message);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <Card
        className="p-4 rounded-2xl shadow-[0_6px_18px_hsl(var(--gal-shadow))] cursor-pointer hover:shadow-[0_10px_30px_hsl(var(--gal-shadow))] active:scale-[0.98] transition-all duration-150 select-none"
        onClick={handleClick}
      ...
      <AlertDialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <AlertDialogContent className="mx-4 rounded-2xl max-w-[calc(100vw-32px)]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Introduceți noul titlu al întâlnirii.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <Input
            value={newTitle}
            onChange={(e) => setNewTitle(e.target.value)}
            placeholder="Titlu întâlnire..."
          ...
          <AlertDialogFooter className="gap-2 sm:gap-0">
            <AlertDialogCancel className="h-12 rounded-xl" onClick={() => setEditDialogOpen(false)}>
              Anulează
            </AlertDialogCancel>
            <AlertDialogAction
              ...
              data-testid="meeting-edit-save-button"
            >
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent className="mx-4 rounded-2xl max-w-[calc(100vw-32px)]">
          ...
          <AlertDialogFooter className="gap-2 sm:gap-0">
            <AlertDialogCancel className="h-12 rounded-xl" onClick={() => setDeleteDialogOpen(false)}>
              Anulează
            </AlertDialogCancel>
            <AlertDialogAction
